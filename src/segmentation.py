...
def train_segmentation_net(
    ...
    if dataset == 'acdc':
        ...
        train_ds = BrainDataset(
            os.path.join(dataset_root, 'Task01_BrainTumour'), train=True
        )
        val_ds = BrainDataset(
            os.path.join(dataset_root, 'Task01_BrainTumour'), train=False
        )
    elif dataset == 'knee':
    ...
    for ep in tqdm(range(1, epochs + 1)):
        ...
        with torch.no_grad():
            ...
            if val_loss <= best_loss:
                ...
                best_loss = val_loss
                tqdm.write(
                    'New best loss: {:.3f}\t|\tCheckpoint saved in epoch {:4d}.'.format(
                        best_loss, ep
                    )
                )
